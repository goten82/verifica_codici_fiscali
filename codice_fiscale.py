...
def encode_lastname(lastname: str) -> str:
    ...
    :rtype: string
    """
    lastname_consonants = _get_consonants(lastname)
    lastname_vowels = _get_vowels(lastname)
...
def encode_firstname(firstname: str) -> str:
    ...
    :rtype: string
    """
    firstname_consonants = _get_consonants(firstname)

    if len(firstname_consonants) > 3:
        del firstname_consonants[1]

    firstname_vowels = _get_vowels(firstname)
...
def decode(code: str) -> dict[str, Any]:
    ...
    birthdate = _get_date(birthdate_str)
    if not birthdate:
...
def is_valid(code: str) -> str:
    ...
    try:
        decode(code)
        return f"{code} - Codice fiscale valido"
    except ValueError:
        return f"{code} - Codice fiscale errato"
